...
# This Hides the Pygame Support Readout
...
def stopMusic():
    if(root.songAdded == False):
        root.screenMessage.set("First add some Music Files!")
    else:
...
subMenu.add_separator()
subMenu.add_command(label="Exit Application", command=exit)

#---------Creating Tri-color Labels----------
one = Label(root, text="Welcome", bg="#111931",fg="white")
one.pack(fill=X)
two = Label(root, text="Will OS Music Player", bg="#111931",fg="white")
two.pack(fill=X)
three = Label(root, text="Will Anderson", bg="#111931",fg="white")
three.pack(fill=X)
...
button1 = Button(topFrame, text="Previous", fg="black", command=prevMusic) #Positioning Button as (<FrameName>,<Text to Appear>,<Text Color>)
button1.pack(side=LEFT,padx=5,pady=20)
button2 = Button(topFrame, text="Play",fg="black", command=playMusic)
...
button5 = Button(topFrame, text="Stop",fg="black", command=stopMusic)
button5.pack(side=LEFT,padx=5,pady=20)
button4 = Button(topFrame, text="Next",fg="black", command=nextMusic)
...
root.screenMessage.set("Welcome, to Will OS Music Player")
